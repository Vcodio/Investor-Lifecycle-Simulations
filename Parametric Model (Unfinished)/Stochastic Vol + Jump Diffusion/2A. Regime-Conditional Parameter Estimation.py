...
try:
    ...
    # Import key functions and constants (match standalone 2B exactly)
    ...
    NUM_RESTARTS = moment_matching.NUM_RESTARTS
    MAXITER = moment_matching.MAXITER
...
def infer_regimes_hmm(returns, features='vol&skew', num_regimes=3, random_seed=42, n_iter=100, tol=1e-3):
    """
    ...
    # Fit HMM (reduced iterations for speed - typically converges in <50 iterations)
...
def estimate_regime_conditional_parameters(returns, regime_labels, num_regimes=3,
    ...
    for regime_id in range(num_regimes):
        ...
        # Regime 3 (extreme crisis): same filtering as standalone 2B main (lines 1334-1364)
        if regime_id == 3 or regime_id == 3.0:
        ...
        if len(regime_returns) < 10:  # Match standalone 2B (line 1366)
            console.print(f"[yellow]⚠ Regime {regime_id}: Only {len(regime_returns)} observations, skipping[/yellow]")
            continue

        # Log-return conversion if needed (match standalone 2B lines 1376-1381)
        ...
        if method == 'moment':
            # Match standalone 2B exactly: name, restarts, maxiter, bounds_vec; match_max_dd=False (standalone default)
            result = fit_bates_moment_matching(
                regime_returns,
                name=f"regime_{float(regime_id)}",
