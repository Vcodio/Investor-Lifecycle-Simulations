...
def compute_empirical_moments(returns):
    ...
    # Compute max drawdown from log returns: P_t = exp(cumsum(returns))
...
def compute_model_moments_simulation(params, n_simulations=500, n_periods=100, seed=None):
    """Compute Bates model moments via Monte Carlo simulation (VECTORIZED for speed).
    
    This replaces the broken analytical formulas with accurate simulation-based computation.
    The analytical formulas for skewness/kurtosis were producing errors of 500-17000%!
    
    VECTORIZED: Simulates all paths in parallel using numpy (~50x faster than nested loops).
    
    Args:
        params: Bates model parameters (9-element array)
        n_simulations: Number of simulation paths (default 500 for speed during optimization)
        n_periods: Number of periods per path (default 100)
    ...
    sqrt_one_minus_rho2 = math.sqrt(1.0 - rho**2)
    
    # Pre-allocate returns array
    all_returns = np.zeros((n_simulations, n_periods))
    
    # Initialize variance at equilibrium for all paths
    variance = np.full(n_simulations, theta)
    
    # Vectorized time loop (paths in parallel, time sequential - required by variance process)
    # IMPORTANT: Use V_t for returns FIRST, then update variance for next period
    for t in range(n_periods):
        # Generate all random numbers at once for this timestep
        z1 = rng.standard_normal(n_simulations)
        z2 = rho * z1 + sqrt_one_minus_rho2 * rng.standard_normal(n_simulations)
        
        # Use current variance (V_t) for this period
        v_t = np.maximum(variance, 1e-12)
        sqrt_var_t = np.sqrt(v_t)
        
        # Merton jumps (vectorized)
        num_jumps = rng.poisson(lam * dt, n_simulations)
        jump_sizes = np.zeros(n_simulations)
        has_jumps = num_jumps > 0
        if np.any(has_jumps):
            total_jumps = num_jumps[has_jumps].sum()
            all_jump_vals = rng.normal(mu_J, sigma_J, total_jumps)
            idx = 0
            for i in np.where(has_jumps)[0]:
                n_j = num_jumps[i]
                jump_sizes[i] = all_jump_vals[idx:idx+n_j].sum()
                idx += n_j
        
        # Log returns - use V_t (correct Euler discretization)
        all_returns[:, t] = (mu - 0.5 * v_t) * dt + sqrt_var_t * sqrt_dt * z1 + jump_sizes
        
        # NOW update variance for next period (V_{t+1})
        variance = v_t + kappa * (theta - v_t) * dt + nu * sqrt_var_t * sqrt_dt * z2
        variance = np.maximum(variance, 1e-12)
    
    # Flatten all returns for moment calculation
    all_returns_flat = all_returns.flatten()
    
    return {
        'mean': float(np.mean(all_returns_flat)),
        'std': float(np.std(all_returns_flat, ddof=0)),
        'skew': float(stats.skew(all_returns_flat, bias=False)) if len(all_returns_flat) > 2 else 0.0,
        'kurt': float(stats.kurtosis(all_returns_flat, fisher=True, bias=False)) if len(all_returns_flat) > 3 else 0.0
    }


def compute_model_moments_analytical(params, v0_override=None, _debug_log_func=None, _debug_context=""):
    """Compute Bates model moments analytically.
    
    WARNING: The analytical skewness and kurtosis formulas are APPROXIMATIONS that have been shown
    to produce errors of 500-17000% compared to actual simulations! 
    Use compute_model_moments_simulation() instead for accurate moment computation.
    
    This function is kept for backward compatibility but should NOT be used for moment matching.
    ...
    Returns:
        dict with keys: 'mean', 'std', 'skew', 'kurt'
        - WARNING: 'skew' and 'kurt' are highly inaccurate approximations!
    """
    mu = params[IDX['mu']]
    kappa = params[IDX['kappa']]
    theta = params[IDX['theta']]
    ...
    # Skewness and kurtosis - USE SIMULATION-BASED VALUES INSTEAD
    # The formulas below are known to be highly inaccurate (500-17000% error)
    # We now use simulation to get accurate skew/kurt
    sim_moments = compute_model_moments_simulation(params, n_simulations=200, n_periods=50, seed=42)
    model_skew = sim_moments['skew']
    model_kurt = sim_moments['kurt']
...
def moment_matching_objective(params, emp_moments, returns, likelihood_weight=LIKELIHOOD_WEIGHT, match_max_dd=None, use_simulation=True):
    ...
    Args:
        ...
        use_simulation: if True, use simulation-based moments (accurate but slower)
    ...
    try:
        ...
        # Feller condition (with small margin for numerical stability)
        feller_gap = 2.0 * kappa * theta - nu ** 2
        if feller_gap <= 0.001:  # Small positive margin
            return 1e12
        
        # Calculate model moments using simulation (accurate) or analytical (fast but inaccurate)
        if use_simulation:
            # Use fast simulation (fewer samples for optimization speed)
            model_moments = compute_model_moments_simulation(params, n_simulations=100, n_periods=50, seed=42)
        else:
            ...
            for attempt in range(restarts):
                ...
                try:
                    res = minimize(
                        ...
                        x0,
                        args=(emp_moments, returns, LIKELIHOOD_WEIGHT, match_max_dd, True),  # use_simulation=True
                        method='L-BFGS-B',
                        bounds=bounds,
                        options=opt_options
                    )
                    ...
                    if DEBUG and attempt == 0:
                        console.print(f"[dim]  First attempt result: success={res.success}, fun={res.fun:.4e}, message={res.message}[/dim]")
                    ...
                    if res.success and res.fun < best_val:
                        ...
                        if best_val < EARLY_STOP_MOMENT_ERROR:
                            ...
                            break
                    elif DEBUG and not res.success:
                        console.print(f"[dim]  Attempt {attempt+1}: optimization failed - {res.message}[/dim]")
                except Exception as e:
                    if DEBUG:
                        console.print(f"[dim]  Attempt {attempt+1} exception: {e}[/dim]")
                    continue
...
def main():
    ...
    parser.add_argument('--portfolio', type=str, default='S&P 500',
                       help='Portfolio name for plot titles and filenames (default: "S&P 500")')
    ...
    # Convert to log returns for consistency with Bates model (which outputs log returns)
    if 'Total Nominal Return (%)' in df.columns:
        simple_ret = df['Total Nominal Return (%)'].values / 100.0
    elif 'returns' in df.columns:
        simple_ret = df['returns'].values
    else:
        console.print("[red]Error: Could not find returns column[/red]")
        return
    df['returns'] = np.log(1.0 + simple_ret)
    ...
    with Progress(
        ...
        for regime_id, regime_df in regimes:
            ...
            log_return_conversion_triggered = False
            if np.any(np.abs(regime_returns) > 0.5):
                # Likely simple returns, convert to log
                log_return_conversion_triggered = True
    ...
    if not args.no_plots:
        ...
        generate_distribution_plots(results, df, regime_col, portfolio_name=args.portfolio)
...
def simulate_bates_returns(params, n_periods, n_simulations=1000, seed=None):
    """Simulate Bates model returns for visualization (VECTORIZED across paths).
    
    Time evolution is sequential (required by variance process), but all paths
    are simulated in parallel using numpy broadcasting for ~50x speedup.
    
    Returns:
        tuple: (returns array [n_simulations x n_periods], volatilities array [n_simulations x n_periods])
    """
    rng = np.random.default_rng(seed)
    mu = params[IDX['mu']]
    kappa = params[IDX['kappa']]
    ...
    # Pre-allocate arrays
    all_returns = np.zeros((n_simulations, n_periods))
    all_volatilities = np.zeros((n_simulations, n_periods))
    
    # Initialize variance for all paths
    variance = np.full(n_simulations, v0)
    
    # Vectorized time loop (paths in parallel, time sequential)
    # IMPORTANT: Use V_t for returns FIRST, then update variance for next period
    for t in range(n_periods):
        # Generate all random numbers at once for this timestep
        z1 = rng.standard_normal(n_simulations)
        ...
        # Use current variance (V_t) for this period's calculations
        v_t = np.maximum(variance, 1e-12)
        sqrt_var_t = np.sqrt(v_t)
        
        # Store instantaneous volatility (before update)
        all_volatilities[:, t] = sqrt_var_t
        ...
        # Bates price process - use V_t (correct Euler discretization)
        log_returns = (mu - 0.5 * v_t) * dt + sqrt_var_t * sqrt_dt * z1 + jump_sizes
        all_returns[:, t] = log_returns
...
def _compute_max_drawdown(returns):
    """Compute max drawdown from log returns (price-space: (P-peak)/peak)."""
    if len(returns) == 0:
        return 0.0
    cumulative_price = np.exp(np.cumsum(returns))
    peak_series = np.maximum.accumulate(cumulative_price)
...
def _generate_regime_metric_comparison(results, regimes_dict, portfolio_name, portfolio_slug, n_simulations_override=None):
    """Same format as return distributions: Empirical histogram + Model histogram for Vol and Max DD."""
    n_regimes = len(results)
    
    fig, axes = plt.subplots(2, n_regimes, figsize=(6 * n_regimes, 10), facecolor='black')
    if n_regimes == 1:
        axes = axes.reshape(2, 1)
    
    fig.suptitle(f'{portfolio_name} - Bates Model: Volatility & Max Drawdown Distributions by Regime', 
    ...
    for col_idx, r in enumerate(results):
        ...
        w = min(21, max(3, n_periods // 4))
        
        n_sims = n_simulations_override if n_simulations_override is not None else min(5000, max(2000, n_periods * 30))
        sim_returns, _ = simulate_bates_returns(params, n_periods, n_simulations=n_sims, seed=42)
        
        # --- Volatility: rolling vol at each time step (identical format to return dist) ---
        emp_vol = pd.Series(emp_returns).rolling(window=w, min_periods=1).std().values * math.sqrt(PERIODS_PER_YEAR)
        emp_vol = emp_vol[~np.isnan(emp_vol)]
        
        model_vols = []
        for i in range(n_sims):
            v = pd.Series(sim_returns[i, :]).rolling(window=w, min_periods=1).std().values * math.sqrt(PERIODS_PER_YEAR)
            model_vols.extend(v[~np.isnan(v)])
        model_vols = np.array(model_vols)
        
        ax_vol = axes[0, col_idx]
        bins_vol = np.linspace(min(np.min(emp_vol), np.min(model_vols)),
                               max(np.max(emp_vol), np.max(model_vols)), 50)
        ax_vol.hist(emp_vol, bins=bins_vol, alpha=0.6, label='Empirical', density=True, color='cyan', edgecolor='white', linewidth=0.5)
        ax_vol.hist(model_vols, bins=bins_vol, alpha=0.6, label='Bates Model', density=True, color='orange', edgecolor='white', linewidth=0.5)
        ax_vol.set_xlabel('Volatility (Annualized)', fontsize=11, color='white')
        ax_vol.set_ylabel('Density', fontsize=11, color='white')
        ax_vol.set_title(f'{regime_name}: Volatility Distribution (n={r["n_obs"]})', fontsize=12, fontweight='bold', color='white')
        ax_vol.legend(facecolor='black', edgecolor='white', labelcolor='white')
        ax_vol.grid(True, alpha=0.3, color='gray')
        ax_vol.set_facecolor('black')
        ax_vol.tick_params(colors='white')
        
        # --- Max Drawdown: bootstrap empirical to get distribution; model = one per path ---
        n_boot = min(2000, max(500, n_periods * 15))
        emp_max_dds = np.array([_compute_max_drawdown(rng.choice(emp_returns, size=n_periods, replace=True)) for _ in range(n_boot)])
        model_max_dds = np.array([_compute_max_drawdown(sim_returns[i, :]) for i in range(n_sims)])
        
        ax_dd = axes[1, col_idx]
        bins_dd = np.linspace(min(np.min(emp_max_dds), np.min(model_max_dds)),
                              max(np.max(emp_max_dds), np.max(model_max_dds)), 50)
        ax_dd.hist(emp_max_dds, bins=bins_dd, alpha=0.6, label='Empirical (bootstrap)', density=True, color='cyan', edgecolor='white', linewidth=0.5)
        ax_dd.hist(model_max_dds, bins=bins_dd, alpha=0.6, label='Bates Model', density=True, color='orange', edgecolor='white', linewidth=0.5)
        ax_dd.set_xlabel('Max Drawdown', fontsize=11, color='white')
        ax_dd.set_ylabel('Density', fontsize=11, color='white')
        ax_dd.set_title(f'{regime_name}: Max Drawdown Distribution (n={r["n_obs"]})', fontsize=12, fontweight='bold', color='white')
        ax_dd.legend(facecolor='black', edgecolor='white', labelcolor='white')
        ax_dd.grid(True, alpha=0.3, color='gray')
        ax_dd.set_facecolor('black')
        ax_dd.tick_params(colors='white')
    
    plt.tight_layout()
    plt.savefig(f'output/{portfolio_slug}_bates_regime_metrics.png', dpi=300, bbox_inches='tight', facecolor='black')
    plt.close()

def generate_distribution_plots(results, df, regime_col, portfolio_name='S&P 500', n_simulations_for_plots=None):
    """Generate all distribution visualizations and save to output folder. If n_simulations_for_plots is set, use it for all simulation calls."""
    os.makedirs('output', exist_ok=True)

    # Create sanitized portfolio name for filenames
    ...
    # 1. Individual regime histograms (Return Distributions)
    n_regimes = len(results)
    fig, axes = plt.subplots(n_regimes, 1, figsize=(14, 4 * n_regimes), facecolor='black')
    if n_regimes == 1:
        axes = [axes]
    
    fig.suptitle(f'{portfolio_name} - Bates Model: Return Distributions by Regime', 
                 fontsize=16, fontweight='bold', color='white', y=1.02)
    ...
    for idx, r in enumerate(results):
        ...
        n_periods = len(emp_returns)
        n_sims = n_simulations_for_plots if n_simulations_for_plots is not None else (min(10000, max(5000, n_periods * 50)) if n_periods < 200 else 10000)
    ...
    plt.tight_layout()
    plt.savefig(f'output/{portfolio_slug}_bates_regime_distributions.png', dpi=300, bbox_inches='tight', facecolor='black')
    plt.close()
    
    # 1B. Regime comparison: Mean, Variance, and Max Drawdown distributions across simulated paths
    _generate_regime_metric_comparison(results, regimes_dict, portfolio_name, portfolio_slug, n_simulations_override=n_simulations_for_plots)
    
    # 2. Joint distribution plots (Returns vs Volatility)
    fig = plt.figure(figsize=(16, 5 * n_regimes), facecolor='black')
    fig.suptitle(f'{portfolio_name} - Bates Model: Joint Return-Volatility Distributions', 
                 fontsize=16, fontweight='bold', color='white', y=1.02)
    ...
    for idx, r in enumerate(results):
        ...
        n_periods = len(emp_returns)
        n_sims = n_simulations_for_plots if n_simulations_for_plots is not None else (min(5000, max(2000, n_periods * 30)) if n_periods < 200 else 5000)
        sim_returns, sim_vols = simulate_bates_returns(params, n_periods, n_simulations=n_sims, seed=42)
    ...
    plt.savefig(f'output/{portfolio_slug}_bates_joint_distributions.png', dpi=300, bbox_inches='tight', facecolor='black')
    plt.close()
    
    # 3. Signature-based volatility analysis
    fig, axes = plt.subplots(n_regimes, 2, figsize=(16, 5 * n_regimes), facecolor='black')
    fig.suptitle(f'{portfolio_name} - Signature-Based Volatility Analysis', 
                 fontsize=16, fontweight='bold', color='white', y=1.02)
    if n_regimes == 1:
    ...
    plt.tight_layout()
    plt.savefig(f'output/{portfolio_slug}_bates_signature_volatility.png', dpi=300, bbox_inches='tight', facecolor='black')
    plt.close()
    
    # 4. Combined comparison plot (Distribution + Q-Q only)
    fig = plt.figure(figsize=(14, 5 * n_regimes), facecolor='black')
    fig.suptitle(f'{portfolio_name} - Bates Model: Distribution & Q-Q Comparison', 
                 fontsize=16, fontweight='bold', color='white', y=1.02)
    gs = gridspec.GridSpec(n_regimes, 2, figure=fig, hspace=0.4, wspace=0.3)
    ...
    for idx, r in enumerate(results):
        ...
        # Simulate model
        n_periods = len(emp_returns)
        n_sims = n_simulations_for_plots if n_simulations_for_plots is not None else (min(8000, max(3000, n_periods * 40)) if n_periods < 200 else 8000)
        sim_returns, _ = simulate_bates_returns(params, n_periods, n_simulations=n_sims, seed=42)
        sim_returns_flat = sim_returns.flatten()
        ...
        # Plot 2: Q-Q plot
        ax2 = fig.add_subplot(gs[idx, 1])
        quantiles = np.linspace(0, 1, min(len(emp_returns), len(sim_returns_flat)))
    ...
    plt.savefig(f'output/{portfolio_slug}_bates_comprehensive_comparison.png', dpi=300, bbox_inches='tight', facecolor='black')
    plt.close()
    
    console.print(f"[green]  ✓ Generated 5 visualization files in output/ folder[/green]")
    console.print(f"[dim]    - {portfolio_slug}_bates_regime_distributions.png[/dim]")
    console.print(f"[dim]    - {portfolio_slug}_bates_regime_metrics.png (Vol, Max DD)[/dim]")
    console.print(f"[dim]    - {portfolio_slug}_bates_joint_distributions.png[/dim]")
    console.print(f"[dim]    - {portfolio_slug}_bates_signature_volatility.png[/dim]")
    console.print(f"[dim]    - {portfolio_slug}_bates_comprehensive_comparison.png[/dim]")
