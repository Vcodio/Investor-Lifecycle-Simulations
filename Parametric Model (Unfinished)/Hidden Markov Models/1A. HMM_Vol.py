...
ANNUALIZATION_FACTOR = 12 # 4 quarters per year

# Define the canonical regime labels. These will be dynamically assigned based on volatility ranking.
CANONICAL_VOL_RANKING = [
    'V-1: Lowest Volatility',
    'V-2: Medium Volatility',
    'V-3: High Volatility',
]
# Defines the desired output index for the final classification (0=Low, 1=Medium, 2=High)
DESIRED_VOL_INDEX = {
    'V-1: Lowest Volatility': 0,
    'V-2: Medium Volatility': 1,
    'V-3: High Volatility': 2,
}
...
if __name__ == "__main__":
    ...
    # Optional: python 1A. HMM_Vol.py <input_csv> [output_dir]  (output_dir = where to write regime CSV and transition matrix)
    _script_dir = Path(__file__).resolve().parent
    file_path = _script_dir / "../../data/Market and Inflation - Regime Testing.csv"
    file_path = str(file_path.resolve())
    if len(sys.argv) > 1:
        file_path = os.path.abspath(sys.argv[1])
    if len(sys.argv) > 2:
        _output_data_dir = os.path.abspath(sys.argv[2])
    else:
        _output_data_dir = str((_script_dir / "../../data").resolve())
    df = None
    loaded = False
    ...
    if df is not None:
        ...
        # 4. HMM Feature: Monthly (21-day) Annualized Volatility of Real Returns
        # Calculate rolling 21-day standard deviation (daily vol) and annualize it.
        ...
        if DEBUG_MODE:
            ...
            console.print(df_monthly[['Volatility']].head())
        ...
        # Final feature used for HMM: Volatility only (1D feature space)
        X_data = df_monthly[['Volatility']].values
        console.print(f"HMM Input Features Shape: {X_data.shape}.")
        console.print(f"  Feature: Non-Overlapping Monthly Annualized Volatility")
        ...
        if X_data.shape[0] < NUM_REGIMES:
            ...
            # --- 3. HMM INFERENCE ---
            console.print("\n--- Running HMM to Infer Regimes (Volatility Only) ---", style="bold yellow")
            ...
            if DEBUG_MODE:
                ...
                for i in range(NUM_REGIMES):
                    ...
                    vol_variance = float(model_inferred.covars_[i, 0])
                    console.print(f"  Regime {i}: Mean Volatility = {mean_vol:.4f}")
                    console.print(f"            Vol Variance = {vol_variance:.4f}")
    
            # --- DYNAMICALLY MAP HMM INDICES (0-2) TO VOLATILITY RANKING ---
    
            # 1. Collect HMM state means (volatility only)
            regime_data = []
            for i in range(NUM_REGIMES):
                regime_data.append({
                    'original_index': i,
            ...
            # 2. Sort regimes by volatility (lowest to highest)
            regime_data.sort(key=lambda x: x['mean_vol'], reverse=False)
            
            if DEBUG_MODE:
                console.print("\n[bold yellow]DEBUG:[/bold yellow] Regime sorting (by volatility - lowest to highest):")
                for i, data in enumerate(regime_data):
                    console.print(f"  Sorted position {i}: Original index {data['original_index']}, "
                                f"Mean Vol={data['mean_vol']:.2f}")
    
            # 3. Assign the new volatility-ranked labels AND the new 0, 1, 2 index
            # With 3 regimes: 0=Lowest Vol, 1=Medium Vol, 2=High Vol
            REGIME_LABELS = {}
            VOL_RANKED_INDEX_MAP = {} # {Original HMM Index: New Volatility Index (0, 1, 2)}
            sorted_regime_presentation_data = [] # For presentation tables
    
            for i, data in enumerate(regime_data):
                # i = 0 -> V-1: Lowest Volatility
                # i = 1 -> V-2: Medium Volatility
                # i = 2 -> V-3: High Volatility
                vol_rank_label = CANONICAL_VOL_RANKING[i]
                final_vol_index = DESIRED_VOL_INDEX[vol_rank_label] # This is the desired 0, 1, 2
                original_hmm_index = data['original_index']
    
                # Map 1: Original HMM Index -> New Volatility Index (0, 1, 2)
            ...
            df_monthly.loc[:, 'Inferred_Regime_VolRanked'] = df_monthly['Original_Inferred_Regime'].map(VOL_RANKED_INDEX_MAP)
            ...
            if DEBUG_MODE:
                ...
                console.print(df_monthly[['Original_Inferred_Regime', 'Inferred_Regime_VolRanked']].head())
    
            # --- MAP MONTHLY REGIME BACK TO DAILY INDEX FOR RISK CALCULATION AND VISUALIZATION ---
            regime_map = df_monthly['Inferred_Regime_VolRanked'].to_dict()
            ...
            # A. Inferred Transition Matrix (P) - SORTED BY VOLATILITY/CRISIS RANK (0, 1, 2, 3)
            console.print(f"\n--- A. Inferred Transition Probability Matrix (P) (Monthly/21-Day Step) - Sorted by Volatility Rank (0, 1, 2) ---", style="bold blue")
            ...
            sorted_labels = [f"{data['final_index']}: {data['Label']}" for data in sorted_regime_presentation_data] # Label with the new 0, 1, 2, 3 index
            ...
            os.makedirs(_output_data_dir, exist_ok=True)
            transmat_output_file = os.path.join(_output_data_dir, "HMM_transition_matrix.csv")
            ...
            for data in sorted_regime_presentation_data:
                ...
                # Handle edge case where p_ii = 1.0 (regime never transitions)
                # In this case, duration is infinite
                if p_ii >= 1.0 - 1e-10:  # Very close to 1.0 or exactly 1.0
                    mean_duration_steps = np.inf
                    std_dev_duration_steps = np.inf
                    ann_mean_duration = np.inf
                else:
            ...
            console.print(f"\n--- B. Regime Duration Statistics (Endogenous Window - Monthly/21-Day) - Sorted by Volatility Rank (0, 1, 2) ---", style="bold blue")
            ...
            for index, row in df_metrics.iterrows():
                # Format infinity values properly
                mean_dur_str = "inf" if np.isinf(row['Mean Duration (Steps/Months)']) else f"{row['Mean Duration (Steps/Months)']:.3f}"
                mean_dur_years_str = "inf" if np.isinf(row['Mean Duration (Years)']) else f"{row['Mean Duration (Years)']:.3f}"
                std_dur_str = "inf" if np.isinf(row['Std Dev Duration (Steps/Months)']) else f"{row['Std Dev Duration (Steps/Months)']:.3f}"
            ...
            for data in sorted_regime_presentation_data:
                ...
                # NOTE: Must filter the daily data using the NEW 'Visual_Regime' column which holds the 0, 1, 2, 3 index.
                ...
                # Count monthly samples in this regime
                monthly_sample_count = (df_monthly['Inferred_Regime_VolRanked'] == final_index).sum()
                daily_sample_count = len(regime_daily_data)
                ...
                means_data.append({
                    ...
                    'N (Monthly)': monthly_sample_count,
                    'N (Daily)': daily_sample_count,
            ...
            table_d.add_column('N (Monthly)', justify='right', style='bold green')
            table_d.add_column('N (Daily)', justify='right', style='dim green')
            ...
            os.makedirs(_output_data_dir, exist_ok=True)
            output_file_name = os.path.join(_output_data_dir, 'regime_classification_nominal_returns.csv')
            df_export.to_csv(output_file_name, index=False)
    
            # Write HMM means for app/consumers (means_ shape (n_regimes, 1) for vol-only)
            import json
            means_for_export = [[float(d['mean_vol'])] for d in sorted_regime_presentation_data]
            with open(os.path.join(_output_data_dir, 'hmm_means.json'), 'w') as f:
                json.dump({'means': means_for_export, 'num_regimes': NUM_REGIMES}, f)
            ...
            if ENABLE_PLOT:
                ...
                # Define colors for the three regimes (brighter for line visibility)
                colors = ['#00BFFF', '#FFD700', '#FF4500'] # 0=DeepSkyBlue, 1=Gold, 2=OrangeRed
                    ...
                    for i in range(len(regime_data) - 1):
                        ...
                        if pd.notna(regime):
                            ...
                            if regime_index < NUM_REGIMES:
                                segment_colors.append(colors[regime_index])
                            else:
                ...
                # Prepare x-axis data as numeric for LineCollection
                x_numeric = np.arange(len(df))
                
                # --- PLOT 1: Volatility ---
                plot_colored_line(ax1, x_numeric, df['Volatility'].values, df['Visual_Regime'], linewidth=1.5)
                ...
                ax1.set_xticklabels([df.index[i].strftime('%Y') for i in tick_indices], rotation=45)
    
                # --- PLOT 2: Price (Log Scale) ---
                plot_colored_line(ax2, x_numeric, df[ASSET_PRICE_COLUMN_NAME].values, df['Visual_Regime'], linewidth=1.5)
                ...
                # Set x-axis ticks for price plot
                ax2.set_xticks(tick_indices)
                ax2.set_xticklabels([df.index[i].strftime('%Y') for i in tick_indices], rotation=45)
    
                # Add regime legend to the price plot
                from matplotlib.lines import Line2D
                legend_elements = [Line2D([0], [0], color=colors[data['final_index']], linewidth=3,
                                         label=f"{data['final_index']}: {data['Label']}")
                                for data in sorted_regime_presentation_data]
                ax2.legend(handles=legend_elements, loc='upper left', title='Inferred Volatility Regimes (0=Low, 2=High)',
                ...
                fig.suptitle(f'HMM Volatility Regime Detection - {ASSET_PRICE_COLUMN_NAME} - 3 Regimes', color='white', fontsize=16, fontweight='bold', y=0.995)
                
                plt.tight_layout()
                
                # Save plot to project output folder (path relative to this script, not cwd)
                _script_dir = Path(__file__).resolve().parent
                _output_dir = _script_dir.parent.parent / 'output'
                _output_dir.mkdir(parents=True, exist_ok=True)
                plot_filename = _output_dir / f'HMM_Vol_Regimes_{asset_name_safe}_3Regimes.png'
