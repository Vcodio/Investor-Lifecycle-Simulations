...
def _init_worker_process():
    ...
    try:
        ...
        if package_name not in sys.modules:
            ...
            pkg.__path__ = [worker_current_dir]
...
def simulate_withdrawals(start_portfolio, start_age, rng_local, params_annual,
    ...
    # Track cumulative inflation for amortization calculations
    cumulative_inflation_since_start = 1.0
    
    # Track portfolio at year start for amortization
    portfolio_at_year_start = portfolio
    
    debug_enabled = getattr(config, 'debug_amortization', False)
    debug_data = []
    if debug_enabled:
        debug_data.append({
            'event': 'init',
            'start_portfolio': float(start_portfolio),
            'start_age': float(start_age),
            'effective_death_age': float(effective_death_age),
            'use_amortization': config.use_amortization
        })
    
    # Initialize spending - use amortization if enabled, otherwise use fixed spending
    initial_spending_real = None  # Track initial spending for threshold check
    if config.use_amortization:
        # Calculate remaining years at retirement start
        remaining_years = effective_death_age - start_age
        
        # Convert portfolio to real dollars (at start, cumulative_inflation = 1.0)
        principal_real = portfolio_at_year_start / cumulative_inflation_since_start
        
        # Get expected return for amortization
        expected_return = getattr(config, 'amortization_expected_return', None)
        if expected_return is None:
            logger.warning(f"[AMORTIZATION] Expected return not set in config, using fallback 3%")
        ...
        # Store initial spending for threshold check
        initial_spending_real = annual_withdrawal_real
        
        # Convert to nominal (at start, cumulative_inflation = 1.0, so real = nominal)
        current_annual_spending_nominal = annual_withdrawal_real
        current_monthly_spending_nominal = current_annual_spending_nominal / 12.0
        
        if debug_enabled:
            debug_data.append({
                'event': 'initial_withdrawal',
                'principal_real': principal_real,
                'remaining_years': remaining_years,
                'expected_return': expected_return,
                'annual_withdrawal_real': annual_withdrawal_real,
                'annual_withdrawal_nominal': current_annual_spending_nominal,
                'cumulative_inflation': cumulative_inflation_since_start
            })
    else:
        initial_spending_real = spending_annual_real
    ...
    while age_in_months < death_age_months:
        ...
        if (age_in_months % 12) == 0 and age_in_months > start_age_months:
            ...
            # Store portfolio value at the start of the year (before any withdrawals this year)
            portfolio_at_year_start = portfolio

            # Update cumulative inflation
            cumulative_inflation_since_start *= (1.0 + annual_inflation)
            
            # Handle amortization-based withdrawals
            if config.use_amortization:
                # Calculate remaining years
                current_age_years = age_in_months / 12.0
                remaining_years = effective_death_age - current_age_years
                
                # Convert portfolio to real dollars
                principal_real = portfolio_at_year_start / cumulative_inflation_since_start
                ...
                # Calculate amortized withdrawal
                annual_withdrawal_real = calculate_amortized_withdrawal(
                    principal_real, remaining_years, expected_return, desired_bequest_real
                )
                
                # Check if spending dropped below threshold (failure condition)
                threshold_failed = False
                if initial_spending_real is not None:
                    min_threshold = config.amortization_min_spending_threshold * initial_spending_real
                    if annual_withdrawal_real < min_threshold:
                        # Spending dropped below threshold - consider this a failure
                        threshold_failed = True
                        if debug_enabled:
                            debug_data.append({
                                'event': 'threshold_failure',
                                'year': current_age_years,
                                'annual_withdrawal_real': annual_withdrawal_real,
                                'min_threshold': min_threshold,
                                'initial_spending_real': initial_spending_real,
                                'portfolio_nominal': portfolio_at_year_start,
                                'portfolio_real': principal_real,
                                'cumulative_inflation': cumulative_inflation_since_start
                            })
                ...
                if debug_enabled:
                    debug_data.append({
                        ...
                        'cumulative_inflation': cumulative_inflation_since_start,
                        'portfolio_nominal_at_year_start': portfolio_at_year_start,
                        'portfolio_real': principal_real,
                        'remaining_years': remaining_years,
                        'annual_withdrawal_real': annual_withdrawal_real,
                        'annual_withdrawal_nominal': annual_withdrawal_nominal,
                        'min_threshold': min_threshold if initial_spending_real is not None else None
                    })
            else:
                # Fixed spending strategy - just inflate the spending
                current_annual_spending_nominal *= (1.0 + annual_inflation)
                current_monthly_spending_nominal = current_annual_spending_nominal / 12.0
            
            # Social security always inflates
    ...
    # Write debug output if enabled - ALWAYS write even if debug_data is empty
    if debug_enabled:
        try:
            import json
            ...
            debug_entry = {
                'start_portfolio': float(start_portfolio),
                'start_age': float(start_age),
                'success': len(portfolio_history) > 0 and portfolio_history[-1] > 0,
                'final_portfolio': float(portfolio_history[-1]) if len(portfolio_history) > 0 else 0.0,
                'debug_events': debug_data,
                'num_events': len(debug_data),
                'use_amortization': config.use_amortization,
                'total_months_simulated': len(portfolio_history),
                'initial_spending_real': float(initial_spending_real) if initial_spending_real is not None else None
            }
            
            # Append to file (multiple simulations may run) - use line buffering
            with open(debug_file, 'a', encoding='utf-8', buffering=1) as f:
                json_str = json.dumps(debug_entry, indent=2)
                f.write(json_str + '\n\n')
                f.flush()
                os.fsync(f.fileno())
        except Exception as e:
            ...
            try:
                ...
                pass
            # Also try to write a simple text marker
            try:
                with open(debug_file, 'a', encoding='utf-8') as f:
                    f.write(f"[ERROR] Failed to write JSON debug: {e}\n")
            except:
                pass
...
def run_single_accumulation_simulation(sim, config, params, principal_lookup, rng,
    ...
    Note: the old standalone unemployment process has been removed; labor income
    now uses the GKOS nonemployment/scarring process in earnings.py.
    """
...
def run_accumulation_simulations(config, params, principal_lookup, rng, bootstrap_data=None, progress_callback=None):
    ...
    all_earnings_real = []
    max_earnings_paths_to_store = min(config.num_outer, 5000)
    ...
    for sim in sim_range:
        ...
        if sim < max_earnings_paths_to_store:
